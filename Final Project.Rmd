...
## Introduction
##### The data set features student’s performance and how it is affected by variables like Gender, race/ethnicity, parental level of education, Lunch, and test preparation. First, gender is broken down into two components: male and female. Race/ethnicity is broken down into groups A, B, C, D, and E. Parental level of education is broken down by degrees: bachelor’s, some college, master’s, associate’s, and high school. Lunch refers to whether the students had standard or free/reduced lunch before the test. Test preparation refers to whether or not they are completely prepared before the test or not. As for tests, this study looked at math scores, reading scores, and writing scores. The purpose of this paper is to look at the student performance data set and find out how easy it is to lie in statistics. Lying in statistics refers to showing research results in a way that would lead those reading the data to misinterpret it. For example, the results of a study meant to show how effective a diet pill is may say that it works when it doesn’t by using something like p-hacking. P-hacking is when you run multiple models and then only select the one that shows a significant result and publish it without mentioning the other tests that did not show significance. Another way to lie in results is by recoding the data you get to make it look like that data is showing significance. This may be done by erasing numbers and adding new ones, for example. Another way could be to only include outliers that make a large impact on the results. One other way to mislead those who look at the data results would be to take out any covariates that make the results insignificant. These are only a few ways that results can be manipulated to give desired outcomes. This paper will only focus on a few of these and show how easy it is to use them. In this paper, we decided to show ways that lying can be done with our student performance data set. We will also show the correct code we used to correspond with the lying code.

## Results:
### Code Part 1: Test Preparation Course Completion vs Test Scores
...
```
```{r}
STUDY <- read_csv("/Users/Sboudra/OneDrive/Desktop/PSYC_Assignments_Boudra/archive/study_performance.csv")

head(STUDY)
```
##### We started off by creating a Total Score Variable which adds the three different scores together.
```{r}
STUDY1 <- STUDY %>%
...
##### Second, we plotted the relationship between the Test Preparation Course and the Total Score using code that resulted in plot that inaccurately shows the differences in test scores between those who completed the test prepartion course and those who did not. 
```{r}
STUDY2 %>%
...
##### After bootstrapping, a histogram was created for the resampled means of the completed test preparation course group and for the group that did not complete the test preparation course.
```{r}
...
ggplot(resampled_data, aes(x = resampled_means, fill = group)) +
  ...
  theme_minimal()
```
##### The histogram is close to being symmetrical which indicates that the resampled means of the two groups are similar, indicating there are not very significant differences. The overlap between the two groups in the histogram is quite large which also indicates that differences between the two groups are not very significant. 
##### After bootstrapping, we then conducted a t-test to look at the relationship between taking the test preparation course and total test scores. 
```{r}
t.test.course <- t.test(total_score ~ test_preparation_course, data = STUDY2)
t.test.course
```
##### Considering the p-value of this t-test is 0.2191 which greater than the p-value of 0.05, we reject the null hypothesis which means that the differences between the group that completed the test preperation course and the group that didn't is not significant. This matches up with the bootstrapping and histogram conclusions.

### Lunch type vs Test Scores
##### After looking at the relationship between completing the test preparation course and total test scores, we decided to look at how lunch type can impact total test scores. The lunch types are broken down into the standard and reduced groups and be began by filtering the datasets for these two groups.
```{r, echo=FALSE}
# The echo=FALSE in the r chunk makes this NOT appear in the rmarkdown file when it is knit. This code excludes scores from the parameters listed form the data analysis which is skewing and curating results in our favor.
...
LUNCH.SCORES
```
##### From the plot above, it appears as though those who had a free/reduced lunch scored more than those who had a standard lunch on average. 

##### After plotting, we bootstrapped for the two lunch groups.
```{r}
bootstrap <- 1000
...
score_free_reduced
```
##### The two resampled means above of 170.5869 for the standard lunch group test scores and 195.0309 for the free/reduced lunch group test scores show that the second group had a higher mean. This indicates that those who had a free/reduced lunch scored better than those who had a standard lunch.
##### After bootstrapping, we plotted a histogram to look at the two lunch groups and their total test scores.
```{r}
resampled_data <- data.frame(
...
ggplot(resampled_data, aes(x = resampled_means, fill = group)) +
  ...
  theme_minimal()
```
##### In the histogram above, the histograms for both the reduced/free lunch students and the standard lunch students, appear to be quite similar with the standard students histogram having a higher frequency. However, neither of the groups overlapped on the histogram which indicates that there are significant differences between the two groups in terms of total test scores.

##### Our next step to further look at the differences between the two groups in terms of their total test scores was to perform t-tests.
```{r}
t.test.lunch <- t.test(total_score ~ lunch, data = STUDY3)
t.test.lunch
```
##### The t-test yielded a p-value of 2.2 * 10^-16 which shows high significance. This indicates the differences between the test scores of the two groups are significant.

### Correct Code For Part 1

##### We revisted the code used above but this time, we corrected it to give accurate and unmanipulated results. We again started by plotting the relationship between taking the test preparation course and the total test score.
```{r}
...
TEST_PREP_SCORES
```
##### This plot shows that those who completed the test preparation course and those who did not were both able to score high scores on average with there being a bigger range of overall test scores for the group that did not complete the course. 
##### Similarly to what we previously did, we also moved on to bootstrapping next.
```{r}
bootstrap <- 1000
...
true.score.none
```
##### Here, the resamples means are very different from the previous one but the resampled mean of the completed course group is still higher. However, this score is much, much higher than the group that did not complete the course which suggests that there is a bigger difference between the two groups than the incorrect bootstrapping indicated. 

##### For the histogram, we did the same thing as before by plotting the resampled means of the two groups.
```{r}
...
##### This histogram showed a higher frequency on average for the group that did not complete the test preparation course but the group that did complete the course has a wider range for resampled means. There is also no overlap in this histogram which indicates that the differences in test scores between the two groups is quite significant. 
##### Next, a t-test was conducted to look at any significance between the two groups and their test scores. 
```{r}
true.test.lunch <- t.test(total_score ~ test_preparation_course, data = STUDY1)
true.test.lunch
```
#### This t-test yielded a p-value of 2.2 * 10^-16 which shows high significance in the differences between the test scores of the two groups.
...
##### We now move on to looking at the results of the correct code for lunch type and test scores. We start off by bootstrapping.
```{r}
bootstrap <- 1000
...
true_score_free
```
##### The resampled mean for the group who ate a standard lunch is much higher than the resampled mean for the group that ate a free/reduced lunch. This indicates that there are significant differences in total test scores between the two.
...
ggplot(true_resampled_data_lunch, aes(x = true_resampled_means_lunch, fill = group)) +
  ...
  theme_minimal()
```
##### The histogram has a higher frequency and wdier range for the free/reduced group but the fact that there is no overlap between the two groups indicates that there are significant differences between the two groups and their total test scores.
...
study_performance <- read.csv("/Users/Sboudra/OneDrive/Desktop/PSYC_Assignments_Boudra/Preparing data.R")
...
par(mfrow = c(2, 2))
```
##### After that, we created a histogram of the correct reading scores. 
```{r}
...
```
##### This histogram shows a wide range of reading scores and the histogram is not as skewed left.
...
# Next, lets see what happens if we allow the bottom 25% of scores to be replaced by any value in the top 50% of scores.
```{r}
...
```
##### In this histogram, the purple line showing the median of the new lying data is to the right of the original median and the histogram is once again more skewed to the left than the original histogram.

##### Finally, lets see what happens if we allow the bottom 25% to be replaced by any value in the top 75% of scores.
```{r}
